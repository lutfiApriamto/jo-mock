...
/*
  LOGIKA PEMROGRAMAN — auth.service.js
  ---------------------------------------

  === createTokens(user) — private helper ===
  Dipanggil oleh registerUser, login, dan refreshAccessToken.
  Menghasilkan DUA jenis token yang berbeda sifatnya:

  ACCESS TOKEN (JWT, 15 menit):
  - Berisi payload { id, role } — middleware auth membaca ini tanpa query DB
  - Short-lived: jika dicuri, hanya berguna selama 15 menit
  - Dikirim di response body — FE simpan di memory (React state/context), BUKAN localStorage
  - Hilang saat halaman di-refresh → FE harus call /auth/refresh untuk mendapat baru

  REFRESH TOKEN (opaque random, 30 hari):
  - 40 byte = 80 karakter hex → entropy 320-bit, tidak bisa di-brute-force
  - Tidak berisi payload apapun (opaque) — validitas selalu butuh DB lookup
  - Raw token dikirim sebagai HttpOnly cookie — JavaScript FE tidak bisa baca
  - Hashed SHA-256 yang disimpan di DB — jika DB bocor, hash tidak berguna
  - Satu user = satu refresh token (single device). Login dari device lain
    akan overwrite token lama → device pertama otomatis ter-logout saat refresh

  findByIdAndUpdate vs user.save():
  - createTokens memakai findByIdAndUpdate karena user baru dibuat (dari User.create())
    masih fresh dan findByIdAndUpdate lebih atomic untuk update single field
  - refreshAccessToken dan forgotPassword memakai user.save() karena user dokumen
    sudah di-fetch dan di-mutasi dalam satu operasi

  === refreshAccessToken(rawRefreshToken) ===
  Token rotation adalah fitur keamanan kritis:
  - Setiap kali /refresh dipanggil, refresh token LAMA langsung diinvalidasi
  - Refresh token BARU digenerate dan dikembalikan via cookie baru
  - Jika attacker mencuri refresh token dan mencoba pakai SETELAH user legitimate
    sudah refresh: token sudah tidak ada di DB → 401 → attacker gagal
  - Jika attacker pakai token SEBELUM user legitimate refresh: token lama hilang →
    user legitimate akan mendapat 401 saat refresh → tanda ada yang tidak beres

  === logout(rawRefreshToken) ===
  - Idempotent: memanggil logout berkali-kali tidak menghasilkan error
  - Jika token tidak ditemukan (sudah expired atau sudah logout): tidak ada efek
  - findOneAndUpdate dengan { refreshToken: null } → token langsung diinvalidasi di DB
    Jika attacker punya salinan cookie yang sama, mereka tidak bisa refresh lagi

  === registerAdmin ===
  - Tidak mengembalikan token — admin harus login manual setelah ini
  - Endpoint sekali pakai: jika superadmin sudah ada → 409, endpoint tidak berguna lagi

  Penggunaan utils yang sudah ada:
  - signToken(payload, expiry) dari jwtHelper → dipakai di createTokens dan refreshAccessToken
  - sendMail dari config/mailer → dipakai di forgotPassword dan resetPassword
*/
